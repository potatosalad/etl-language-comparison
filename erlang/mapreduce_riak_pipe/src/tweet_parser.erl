...
parse(Binary, SendOutput) ->
	parse(Binary, binary:compile_pattern(<< $\n >>), binary:compile_pattern(<< $\t >>), binary:compile_pattern(?SEARCH), SendOutput).

parse(Binary, Newline, Tab, Search, SendOutput) ->
						...
						{A, _} ->
							case binary:match(Binary, Tab, [{scope, {A+1, Pos-A-1}}]) of
								{B, _} ->
									...
									SendOutput({Hood, 1}),
									parse(binary:part(Binary, Pos+1, byte_size(Binary)-Pos-1), Newline, Tab, Search, SendOutput);
								_ ->
									erlang:error(badarg)
							end;
						_ ->
